...
class HornyDetectionListener(instances: InstanceManager) : ImperiumApplication.Listener {
    ...
    override fun onImperiumInit() {
        ...
        serverCommandManager.buildAndRegister("image-analysis") {
            literal("save-images")
            commandDescription("Whether images generated by the server should be saved to disk")
            argument(BooleanArgument.optional("enabled"))
            handler { ctx ->
                if (ctx.contains("enabled")) {
                    debug = ctx.get("enabled")
                    ctx.sender.sendMessage("Image analysis debug mode is now ${if (debug) "enabled" else "disabled"}")
                } else {
                    ctx.sender.sendMessage("Image analysis debug mode is ${if (debug) "enabled" else "disabled"}")
                }
            }
        }
