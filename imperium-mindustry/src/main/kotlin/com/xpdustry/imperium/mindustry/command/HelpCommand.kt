...
class HelpCommand : ImperiumApplication.Listener {
    ...
    private fun onHelpCommand(sender: CommandSender, @Greedy query: String? = null) {
        ...
        val command = Vars.netServer.clientCommands.commandList.find { it.text == path[0] }
            ?: return sender.sendMessage("[scarlet]No command found.")
        if (command is ArcCommand<*>) {
            onArcCommandHelp(sender, command, command.realName + " " + path.drop(1).joinToString(" "))
        } else {
            sender.sendMessage(
                buildString {
                    appendLine("[orange]-- [lightgray]/${command.text} ${command.paramText} --[orange]")
    ...
    private fun onHelpPageCommand(sender: CommandSender, page: Int) {
        if (page < 0 || page > getPageNumber()) {
            sender.sendMessage("[scarlet]'page' must be a number between[orange] 1[] and[orange] ${getPageNumber()}[scarlet].")
            return
        }
        sender.sendMessage(
            buildString {
                appendLine("[orange]-- Commands Page[lightgray] $page[gray]/[lightgray]${getPageNumber()}[orange] --")
                Vars.netServer.clientCommands.commandList.asSequence()
    ...
    private fun <S : Any> onArcCommandHelp(sender: CommandSender, command: ArcCommand<S>, query: String) {
        ...
        when (val topic = command.manager.createCommandHelpHandler().queryHelp(mapped, query)) {
            is CommandHelpHandler.MultiHelpTopic<S> -> {
                sender.sendMessage(
                    buildString {
                        appendLine("[orange]-- Suggestions for [lightgray]${topic.longestPath}[orange] --")
            ...
            is CommandHelpHandler.VerboseHelpTopic<S> -> {
                sender.sendMessage(
                    buildString {
                        val syntax = command.manager.commandSyntaxFormatter().apply(topic.command.arguments, null)
                        ...
                        for (argument in topic.command.components.drop(1)) {
                            val argumentDescription = argument.argumentDescription.getDescription(locale)
                            if (argumentDescription.isNotBlank()) {
                                appendLine(" [accent]${argument.argument.name}: [lightgray]$argumentDescription")
                            }
                        }
                    },
                )
            ...
            is CommandHelpHandler.IndexHelpTopic<S> -> {
                logger.error("Impossible state: IndexHelpTopic with command ${command.text} and query $query")
    ...
    private fun getPageNumber() = ceil(Vars.netServer.clientCommands.commandList.size.toFloat() / COMMANDS_PER_PAGE).toInt()

    private fun CommandHandler.Command.getDescription(locale: Locale) = if (this is ArcCommand<*>) {
        manager.commands().find { it.components[0].argument.name == realName }!!.getDescription(locale)
    } else {
        description
    }

    private fun cloud.commandframework.Command<*>.getDescription(locale: Locale): String {
        val cloudDescription = components.last { it.argument is StaticArgument<*> }.argumentDescription
        return if (cloudDescription is LocalisableArgumentDescription) {
            cloudDescription.getDescription(locale)
        } else {
            cloudDescription.description.takeUnless(String::isBlank) ?: commandMeta[CommandMeta.DESCRIPTION].orElse("")
        }
    }

    private fun ArgumentDescription.getDescription(locale: Locale): String = when (this) {
