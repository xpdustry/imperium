...
class DdosGatekeeper(private val http: OkHttpClient) :
    ...
    private val addresses: Deferred<Set<InetAddress>> =
        ImperiumScope.MAIN.async(start = CoroutineStart.LAZY) { fetchAddresses() }
    ...
    private suspend fun fetchAddresses(): Set<InetAddress> =
            ...
            PROVIDERS.map { provider ->
                    async {
                        try {
                            ...
                            logger.debug(
                                "Found {} addresses for cloud provider '{}'",
                                result.size,
                                provider.name)
                            result
                        } catch (e: Exception) {
                            logger.error(
                                "Failed to fetch addresses for cloud provider '{}'",
                                provider.name,
                                e)
                            emptyList()
                        }
...
private abstract class JsonAddressProvider protected constructor(override val name: String) :
    ...
    override suspend fun fetchAddresses(http: OkHttpClient): List<InetAddress> {
        ...
        if (response.code != 200) {
            throw IOException(
                "Failed to download '$name' public addresses file (status-code: ${response.code}, url: $url).")
        }
