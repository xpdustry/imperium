...
fun player_afk(removed: Boolean): Component =
    components(
        WHITE,
        text(">>> ", CYAN),
        translatable(
            "imperium.player.afk${if (removed) ".removed" else ".enabled"}",
        ),
    )

fun player_afk_kick(): Component =
    components(
        WHITE,
        translatable("imperium.player.afk.kicked", SCARLET),
    )
