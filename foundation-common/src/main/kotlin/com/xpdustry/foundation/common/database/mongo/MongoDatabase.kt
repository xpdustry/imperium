...
class MongoDatabase @Inject constructor(private val mongo: MongoProvider) : Database, FoundationListener {

    override lateinit var users: UserManager
    override lateinit var punishments: PunishmentManager

    override fun onFoundationInit() {
        users = MongoUserManager(mongo.database.getCollection("users", User::class.java))
        punishments = MongoPunishmentManager(mongo.database.getCollection("punishments", Punishment::class.java))
    }
}
