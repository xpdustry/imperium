...
internal class SimpleLogicImageAnalysis(
    ...
    override suspend fun isUnsafe(
        blocks: List<Cluster.Block<out LogicImage>>
    ): Pair<Boolean, ObjectId?> =
                ...
                is ImageAnalysis.Result.Success -> {
                    ...
                    if (result.details.values.any { it > 0F }) {
                        ...
                        launch {
                            ...
                            storage
                                .getObject("images", "unsafe", "${hashed._id.toHexString()}.jpg")
                                .putData(image.inputStream(ImageFormat.JPG))
                        }
    ...
    override suspend fun findHashedImageById(
        id: ObjectId
    ): Pair<HashedLogicImage, StorageBucket.S3Object>? =
        collection.findById(id)?.let {
            it to storage.getObject("images", "unsafe", "${it._id.toHexString()}.jpg")
        }
    ...
    private suspend fun hash(blocks: List<Cluster.Block<out LogicImage>>): Set<String> =
        withContext(ImperiumScope.IO.coroutineContext) {
                ...
                when (val image = block.data) {
                    ...
                    is LogicImage.Drawer -> {
                        for (processor in image.processors) {
                            ...
                            for (instruction in
                                processor.instructions.sortedWith { a, b ->
                                    Arrays.compare(toIntArray(a), toIntArray(b))
                                }) {
                                when (instruction) {
    ...
    private suspend fun findHashedImageByImages(
        blocks: List<Cluster.Block<out LogicImage>>
    ): Pair<Set<String>, HashedLogicImage?> {
                        ...
                        Document(
                            ...
                            Document(
                                ...
                                listOf(
                                    Document(
                                        "\$divide",
                                        mutableListOf("\$matchedCount", "\$totalCount")),
                                    100L),
                            ),
                        ),
                    ),
        ...
        return if (result != null &&
            (result.getDouble("matchPercentage").value >= 80 ||
                result.getInt32("matchedCount").value == hashes.size)) {
    ...
    private fun toIntArray(instruction: LogicImage.Drawer.Instruction) =
        when (instruction) {
            is LogicImage.Drawer.Instruction.Color ->
                intArrayOf(0, instruction.r, instruction.g, instruction.b, instruction.a)
            is LogicImage.Drawer.Instruction.Rect ->
                intArrayOf(1, instruction.x, instruction.y, instruction.w, instruction.h)
            is LogicImage.Drawer.Instruction.Triangle ->
                intArrayOf(
                    2,
                    instruction.x1,
